...
class ServerlessAutoSwagger {
  ...
  preDeploy = async () => {
    ...
    function find_EXAMPLE(arr: string[]) {
      const itemArr: object[] = [];
      arr.map((item) => {
    ...
    function findEnumValue(propertyObj: Record<string, any>, itemName: string): string {
      return propertyObj[itemName].enum[0];
    }

    function removeProperty(modelAttr: Record<string, any>, itemName: string) {
      delete modelAttr.properties[itemName];
      modelAttr.required = modelAttr.required.filter((item: string) => item !== itemName);
    }

    function setProperty(propertyObj: Record<string, any>, originItem: string, updateItem: string, type: string) {
      propertyObj[originItem][type] = updateItem;
    }

    function addDescAndExample(itemArr: object[], modelAttr: Record<string, any>, type: string) {
      itemArr.map((eachItem: Record<string, any>) => {
        const updateItem = findEnumValue(modelAttr.properties, eachItem[type]);
        removeProperty(modelAttr, eachItem[type]);
        setProperty(modelAttr.properties, eachItem.originItem, updateItem, type);
      });
    }

    const definitions = this.swagger.definitions;
    if (definitions) {
  ...
  generateSwagger = async () => {
    ...
    await fs.copy('./node_modules/serverless-auto-swagger-desc-example/dist/resources', './swagger');
    ...
    if (this.serverless.service.provider.runtime?.includes('python')) {
      const swaggerStr = JSON.stringify(this.swagger, null, 2)
        ...
        .replace(/null/g, 'None');
      let swaggerPythonString = `# this file was generated by serverless-auto-swagger-desc-example`;
      swaggerPythonString += `\ndocs = ${swaggerStr}`;
      ...
      const swaggerJavaScriptString = `// this file was generated by serverless-auto-swagger-desc-example
